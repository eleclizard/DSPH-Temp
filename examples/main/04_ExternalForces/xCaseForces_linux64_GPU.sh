...
${dualsphysicsgpu} -gpu ${dirout}/${name} ${dirout} -dirdataout data -svres -cellmode:2H
if [ $? -ne 0 ] ; then fail; fi
...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:-all,+mk,+vel,+type,+rhop,+press
if [ $? -ne 0 ] ; then fail; fi
