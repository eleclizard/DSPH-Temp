...
${measuretool} -dirin ${diroutdata} -points wg1234.txt -onlytype:-all,+fluid -elevation:0.5 -savecsv ${dirout2}/wg1234  
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/boundary
${boundaryvtk} -loadvtk AutoActual -motiondata ${diroutdata} -savevtkdata ${dirout2}/MotionPiston -onlytype:moving -savevtkdata ${dirout2}/Box.vtk -onlytype:fixed
if [ $? -ne 0 ] ; then fail; fi
