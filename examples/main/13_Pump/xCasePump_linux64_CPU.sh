...
${boundaryvtk} -loadvtk AutoActual -motiondatatime ${diroutdata} -savevtkdata ${dirout2}/MotionPump -onlytype:moving -savevtkdata ${dirout2}/Pumpfixed.vtk -onlytype:fixed
if [ $? -ne 0 ] ; then fail; fi
...
${isosurface} -dirin ${diroutdata} -saveiso ${dirout2}/Surface -vars:-all,vel,rhop
if [ $? -ne 0 ] ; then fail; fi
