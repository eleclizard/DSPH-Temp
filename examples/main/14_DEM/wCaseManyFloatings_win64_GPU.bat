...
set gencaseMkWord="%dirbin%/GenCase_MkWord_win64.exe"
...
%partvtk% -dirin %diroutdata% -savevtk %dirout2%/PartAll -vars:mk
if not "%ERRORLEVEL%" == "0" goto fail

set dirout2=%dirout%\boundary
%boundaryvtk% -loadvtk %dirout%/%name%_MkCells.vtk -motiondata %diroutdata% -savevtkdata %dirout2%/Box.vtk -onlymk:10
if not "%ERRORLEVEL%" == "0" goto fail

%boundaryvtk% -loadvtk %dirout%/%name%_MkCells.vtk -motiondata %diroutdata% -savevtkdata %dirout2%/MotionBlocks -onlymk:11-2999
if not "%ERRORLEVEL%" == "0" goto fail

%boundaryvtk% -loadvtk %dirout%/%name%__Actual.vtk -motiondata %diroutdata% -savevtkdata %dirout2%/MotionSpheres -onlymk:11-2999
if not "%ERRORLEVEL%" == "0" goto fail
