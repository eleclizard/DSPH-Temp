...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartSolids -onlytype:-all,+floating
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/boundary
${boundaryvtk} -loadvtk AutoDp -motiondata ${diroutdata} -savevtkdata ${dirout2}/Tank.vtk -onlymk:10
if [ $? -ne 0 ] ; then fail; fi

${boundaryvtk} -loadvtk AutoDp -motiondata ${diroutdata} -savevtkdata ${dirout2}/Ball -onlymk:19
if [ $? -ne 0 ] ; then fail; fi

${boundaryvtk} -loadvtk AutoDp -motiondata ${diroutdata} -savevtkdata ${dirout2}/Bowls -onlymk:20-89
if [ $? -ne 0 ] ; then fail; fi
