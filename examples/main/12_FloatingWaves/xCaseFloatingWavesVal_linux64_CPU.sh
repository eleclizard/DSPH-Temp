...
export name=CaseFloatingWavesVal
...
${measuretool} -dirin ${diroutdata} -points CaseFloatingWavesVal_elevation.txt -onlytype:-all,+fluid -elevation:0.4 -savecsv ${dirout2}/_${name}_elevation -savevtk ${dirout2}/${name}_elevation
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/forces
${computeforces} -dirin ${diroutdata} -onlymk:60 -savecsv ${dirout2}/_FloatingForce -savevtk ${dirout2}/FloatingForce
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/floatinginfo
${floatinginfo} -dirin ${diroutdata} -onlymk:60 -savedata ${dirout2}/FloatingMotion 
if [ $? -ne 0 ] ; then fail; fi
