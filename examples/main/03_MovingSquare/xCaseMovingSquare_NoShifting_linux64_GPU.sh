...
export name=CaseMovingSquare
export dirout=${name}_NoShifting_out
...
${dualsphysicsgpu} -gpu ${dirout}/${name} ${dirout} -dirdataout data -svres -shifting:none
if [ $? -ne 0 ] ; then fail; fi
...
${boundaryvtk} -loadvtk ${dirout}/${name}__Dp.vtk -motiondata ${diroutdata} -savevtkdata ${dirout2}/Tank.vtk -onlymk:10
if [ $? -ne 0 ] ; then fail; fi

${boundaryvtk} -loadvtk ${dirout}/${name}__Dp.vtk -motiondata ${diroutdata} -savevtkdata ${dirout2}/Square -onlymk:20
if [ $? -ne 0 ] ; then fail; fi
