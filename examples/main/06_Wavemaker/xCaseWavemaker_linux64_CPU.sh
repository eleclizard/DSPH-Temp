...
${boundaryvtk} -loadvtk AutoActual -filexml ${dirout}/${name}.xml -savevtkdata ${dirout2}/MotionPREPiston -onlytype:moving 
if [ $? -ne 0 ] ; then fail; fi
...
export dirout2=${dirout}/boundary
${boundaryvtk} -loadvtk AutoActual -motiondatatime ${diroutdata} -savevtkdata ${dirout2}/MotionPiston -onlytype:moving -savevtkdata ${dirout2}/Box.vtk -onlytype:fixed
if [ $? -ne 0 ] ; then fail; fi
...
${measuretool} -dirin ${diroutdata} -points CaseWavemaker_PointsElevation.txt -onlytype:-all,+fluid -elevation -savevtk ${dirout2}/PointsElevation -savecsv ${dirout2}/_PointsElevation 
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirin ${diroutdata} -points CaseWavemaker_wg0_3D.txt -onlytype:-all,+fluid -elevation -savecsv ${dirout2}/_WG0 
if [ $? -ne 0 ] ; then fail; fi
