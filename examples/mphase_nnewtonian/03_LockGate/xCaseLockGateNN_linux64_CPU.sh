...
# Executes IsoSurface to create VTK files with slices of surface for p1
export dirout2=${dirout}/surface
${isosurface} -dirin ${diroutdata} -saveslice ${dirout2}/Slices_phase1 -onlymk:1
if [ $? -ne 0 ] ; then fail; fi

# Executes IsoSurface to create VTK files with slices of surface for p2.
export dirout2=${dirout}/surface
