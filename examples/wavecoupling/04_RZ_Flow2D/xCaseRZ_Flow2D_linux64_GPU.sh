...
# "name" and "dirout" are named according to the testcase for DualSPHysics and SWASH

export name=CaseRZ_Flow2D
...
${measuretool} -dirin ${diroutdata} -points velocity.txt -onlytype:-all,+fluid -vars:-all,+vel -kclimit:0.4 -savecsv ${dirout2}/Velocity -savevtk ${dirout2}/Velocity
if [ $? -ne 0 ] ; then fail; fi
