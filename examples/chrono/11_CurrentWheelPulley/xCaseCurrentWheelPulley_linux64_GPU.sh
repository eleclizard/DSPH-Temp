...
export name=CaseCurrentWheelPulley
...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartWheel -onlytype:-all,+floating -vars:+mk
if [ $? -ne 0 ] ; then fail; fi
...
${boundaryvtk} -loadvtk AutoDp -motiondata ${diroutdata} -savevtkdata ${dirout2}/Wheel_Gear -onlytype:floating -savevtkdata ${dirout2}/Box.vtk -onlytype:fixed
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/floatinginfo
${floatinginfo} -dirin ${diroutdata} -onlymk:60 -savedata ${dirout2}/Wheel 
if [ $? -ne 0 ] ; then fail; fi

${floatinginfo} -dirin ${diroutdata} -onlymk:61 -savedata ${dirout2}/Gear 
if [ $? -ne 0 ] ; then fail; fi
