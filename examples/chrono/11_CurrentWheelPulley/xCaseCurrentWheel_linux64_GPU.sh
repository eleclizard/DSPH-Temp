...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartWheel -onlytype:-all,+floating
if [ $? -ne 0 ] ; then fail; fi
...
${boundaryvtk} -loadvtk AutoDp -motiondata ${diroutdata} -savevtkdata ${dirout2}/Wheel -onlytype:floating -savevtkdata ${dirout2}/Box.vtk -onlytype:fixed
if [ $? -ne 0 ] ; then fail; fi
