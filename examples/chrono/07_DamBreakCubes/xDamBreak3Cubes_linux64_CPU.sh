...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartCube -onlytype:-all,+floating
if [ $? -ne 0 ] ; then fail; fi
...
${boundaryvtk} -loadvtk AutoDp -motiondata ${diroutdata} -savevtkdata ${dirout2}/Cubes -onlytype:floating -savevtkdata ${dirout2}/Box.vtk -onlytype:fixed
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/floatinginfo
${floatinginfo} -dirin ${diroutdata} -onlymk:61 -savedata ${dirout2}/FloatingMotionCube1 
if [ $? -ne 0 ] ; then fail; fi

${floatinginfo} -dirin ${diroutdata} -onlymk:62 -savedata ${dirout2}/FloatingMotionCube2 
if [ $? -ne 0 ] ; then fail; fi

${floatinginfo} -dirin ${diroutdata} -onlymk:63 -savedata ${dirout2}/FloatingMotionCube3 
if [ $? -ne 0 ] ; then fail; fi
