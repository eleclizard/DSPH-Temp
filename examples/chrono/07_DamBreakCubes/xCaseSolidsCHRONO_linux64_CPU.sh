...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartBlocks -onlytype:-all,+floating 
if [ $? -ne 0 ] ; then fail; fi
...
${boundaryvtk} -loadvtk AutoActual -motiondata ${diroutdata} -savevtkdata ${dirout2}/MotionBlocks -onlytype:floating -savevtkdata ${dirout2}/Box.vtk -onlytype:fixed
if [ $? -ne 0 ] ; then fail; fi
