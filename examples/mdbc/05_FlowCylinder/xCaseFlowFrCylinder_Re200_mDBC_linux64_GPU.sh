...
${measuretool} -dirin ${diroutdata} -points points_cyl.txt -savecsv ${dirout2}/VorPoint_mDBC -vars:-all,+vor 
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/forces
${computeforces} -dirin ${diroutdata} -onlymk:10 -viscolam:0.001 -savecsv ${dirout2}/_ForceCylinder_mDBC
if [ $? -ne 0 ] ; then fail; fi
