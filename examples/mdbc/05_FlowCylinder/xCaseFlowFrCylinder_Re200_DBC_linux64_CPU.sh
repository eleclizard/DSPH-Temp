...
export name=CaseFlowFrCylinder_Re200
export dirout=${name}_DBC_out
...
${measuretool} -dirin ${diroutdata} -points points_cyl.txt -savecsv ${dirout2}/VorPoint_DBC -vars:-all,+vor 
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/forces
${computeforces} -dirin ${diroutdata} -onlymk:10 -viscolam:0.001 -savecsv ${dirout2}/_ForceCylinder_DBC
if [ $? -ne 0 ] ; then fail; fi
