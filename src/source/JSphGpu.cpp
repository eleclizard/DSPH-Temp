...
JSphGpu::JSphGpu(bool withmpi):JSph(false,false,withmpi),DivAxis(MGDIV_None){
  ClassName="JSphGpu";
  Idp=NULL; Code=NULL; Dcell=NULL; Posxy=NULL; Posz=NULL; Velrhop=NULL; Temp=NULL;
  AuxPos=NULL; AuxVel=NULL; AuxRhop=NULL; AuxTemp=NULL;
  CellDiv=NULL;
...
void JSphGpu::AllocGpuMemoryParticles(unsigned np,float over){
  ...
  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B,1); //Atemp
  if(TStep==STEP_Verlet){
    ArraysGpu->AddArrayCount(JArraysGpu::SIZE_16B,1); //-velrhopm1
    ArraysGpu->AddArrayCount(JArraysGpu::SIZE_8B,1); //TempM1
  }
    ...
    ArraysGpu->AddArrayCount(JArraysGpu::SIZE_8B,1); //TempPrec
  }
...
void JSphGpu::ReserveBasicArraysGpu(){
  ...
  PosCellg=ArraysGpu->ReserveFloat4();
  Velrhopg=ArraysGpu->ReserveFloat4();
  Tempg= ArraysGpu->ReserveDouble(); //Array for Tempg
  if(TStep==STEP_Verlet){
    VelrhopM1g=ArraysGpu->ReserveFloat4();
    TempM1g = ArraysGpu->ReserveDouble(); //Array for TempM1g
  }
...
void JSphGpu::ParticlesDataUp(unsigned n,const tfloat3 *boundnormal){
  ...
  printf("Right after copy from host to device for Tempg:\n");
  double *T;
  T = (double*)malloc(sizeof(double)*Np);
  cudaMemcpy(T,Tempg,sizeof(double)*Np,cudaMemcpyDeviceToHost);
  printf("Tempg[1000] = %f\n",T[1000]);
  delete[] T;
  T=NULL;
...
void JSphGpu::InitRunGpu(){
  ...
  if(TStep==STEP_Verlet){
    ...
    cudaMemcpy(TempM1g, Tempg, sizeof(double)*Np, cudaMemcpyDeviceToDevice); //Temperature
  }
...
void JSphGpu::PreInteractionVars_Forces(unsigned np,unsigned npb){
  ...
  cudaMemset(Atempg, 0, sizeof(float)*np);                               //Atempg[]=0
...
void JSphGpu::PreInteraction_Forces(){
  ...
  Atempg=ArraysGpu->ReserveFloat(); //Temperature rate of change
...
void JSphGpu::ComputeVerlet(double dt){  //pdtedom
  ...
  if(VerletStep<VerletSteps){
    cusphs::ComputeStepVerlet(WithFloating,shift,inout,Np,Npb,Velrhopg,VelrhopM1g,Tempg,TempM1g,Arg,Atempg
      ,Aceg,ShiftPosfsg,indirvel,dt,dt+dt,RhopZero,RhopOutMin,RhopOutMax,Gravity,Codeg,movxyg,movzg,VelrhopM1g,TempM1g,NULL);
  }
  else{
    cusphs::ComputeStepVerlet(WithFloating,shift,inout,Np,Npb,Velrhopg,Velrhopg,Tempg,Tempg,Arg,Atempg
      ,Aceg,ShiftPosfsg,indirvel,dt,dt,RhopZero,RhopOutMin,RhopOutMax,Gravity,Codeg,movxyg,movzg,VelrhopM1g,TempM1g,NULL);
    VerletStep=0;
  }
  ...
  swap(Tempg,TempM1g); // Exchange Tempg and TempM1g
...
void JSphGpu::ComputeSymplecticPre(double dt){
  ...
  const float3 *indirvel=(InOut? InOut->GetDirVelg(): NULL);
  cusphs::ComputeStepSymplecticPre(WithFloating,shift,inout,Np,Npb,VelrhopPreg,TempPreg,Arg,Atempg
    ,Aceg,ShiftPosfsg,indirvel,dt05,RhopZero,RhopOutMin,RhopOutMax,Gravity
    ,Codeg,movxyg,movzg,Velrhopg,Tempg,NULL);
  //-Applies displacement to non-periodic fluid particles.
  //-Aplica desplazamiento a las particulas fluid no periodicas.
...
void JSphGpu::ComputeSymplecticCorr(double dt){
  ...
  const double dt05=dt*.5;
  const float3 *indirvel=(InOut? InOut->GetDirVelg(): NULL);
  cusphs::ComputeStepSymplecticCor(WithFloating,shift,inout,Np,Npb,VelrhopPreg,TempPreg
    ,Arg,Atempg,Aceg,ShiftPosfsg,indirvel,dt05,dt,RhopZero,RhopOutMin,RhopOutMax,Gravity
    ,Codeg,movxyg,movzg,Velrhopg,Tempg,NULL);
  //-Applies displacement to non-periodic fluid particles.
  //-Aplica desplazamiento a las particulas fluid no periodicas.
  cusph::ComputeStepPos2(PeriActive,WithFloating,Np,Npb,PosxyPreg,PoszPreg
