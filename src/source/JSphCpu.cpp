...
void JSphCpu::AllocCpuMemoryParticles(unsigned np,float over){
  ...
  // Temperature allocation for Tempc and Atempc
    ...
    for(int z=ngs.zini;z<ngs.zfin;z++)for(int y=ngs.yini;y<ngs.yfin;y++){
      ...
      for(unsigned p2=pif.x;p2<pif.y;p2++){
        ...
        if(rr2<=KernelSize2 && rr2>=ALMOSTZERO){
          ...
          const float frx=fac*drx,fry=fac*dry,frz=fac*drz,fabc=fac; //-Gradients. Additional temperature gradient
    ...
    for(int z=ngs.zini;z<ngs.zfin;z++)for(int y=ngs.yini;y<ngs.yfin;y++){
      ...
      for(unsigned p2=pif.x;p2<pif.y;p2++){
        ...
        if(rr2<=KernelSize2 && rr2>=ALMOSTZERO){
          //-Computes kernel.
          const float fac=fsph::GetKernel_Fac<tker>(CSP,rr2);
          const float frx=fac*drx,fry=fac*dry,frz=fac*drz,fabc=fac; //-Gradients. With additional temp gradient fabc
...
void JSphCpu::ComputeVelrhopBound(const tfloat4* velrhopold, const double *tempold,double armul,tfloat4* velrhopnew, double *tempnew)const{
  ...
  for(int p=0;p<npb;p++){
    ...
    tempnew[p]=tempold[p]; // Temperature: Constant boundary.
  }
}
...
void JSphCpu::ComputeVerlet(double dt){
  ...
  swap(Tempc,TempM1c); // Temperature: Swap new vs old temp.
...
void JSphCpu::ComputeSymplecticPre(double dt){
  ...
  for(int p=npb;p<np;p++){
    ...
    Tempc[p]=TempPrec[p]+dt05*Atempc[p]; //Temperature: Calculate new temperature for fluid.
