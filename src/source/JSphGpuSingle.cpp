...
void JSphGpuSingle::ConfigDomain(){
  ...
  /*printf("Right after Tempg is initiated:\n");
  double *P, *T;
  P = (double*)malloc(sizeof(double)*Np);
  T = (double*)malloc(sizeof(double)*Np);
  cudaMemcpy(P,Poszg,sizeof(double)*Np,cudaMemcpyDeviceToHost);
  ...
  BoundChanged=true;
  RunCellDivide(true);
  /*printf("Right after RunCellDivide is initiated:\n");
  double *P, *T;
  P = (double*)malloc(sizeof(double)*Np);
  T = (double*)malloc(sizeof(double)*Np);
  cudaMemcpy(P,Poszg,sizeof(double)*Np,cudaMemcpyDeviceToHost);
...
void JSphGpuSingle::Interaction_Forces(TpInterStep interstep){
  ...
  const StInterParmsg parms=StrInterParmsg(Simulate2D
    ...
    ,0,DivData,Dcellg
    ,Posxyg,Poszg,PosCellg,Velrhopg,Tempg,Idpg,Codeg
    ,FtoMasspg,SpsTaug
    ,ViscDtg,Arg,Atempg,Aceg,Deltag
    ,SpsGradvelg
    ,ShiftPosfsg
    ,NULL,NULL);
  /*printf("Before cusph::Interaction_Forces:\n");
  double *P, *T;
  P = (double*)malloc(sizeof(double)*Np);
  T = (double*)malloc(sizeof(double)*Np);
  cudaMemcpy(P,Poszg,sizeof(double)*Np,cudaMemcpyDeviceToHost);
  ...
  double *T;
  T = (double*)malloc(sizeof(double)*Np);
  cudaMemcpy(T,parms.temp,sizeof(double)*Np,cudaMemcpyDeviceToHost);
  printf("Tempg[1000] = %f\n",T[1000]);
  delete[] T; T=NULL;
...
double JSphGpuSingle::ComputeStep_Ver(){
  printf("Right Before BoundCorrectionData(): \n");
  double *T;
  T = (double*)malloc(sizeof(double)*Np);
  cudaMemcpy(T,Tempg,sizeof(double)*Np,cudaMemcpyDeviceToHost);
  printf("Tempg[1000] = %f\n",T[1000]);
  ...
  printf("Right Before Interaction_Forces(): \n");
  T = (double*)malloc(sizeof(double)*Np);
  cudaMemcpy(T,Tempg,sizeof(double)*Np,cudaMemcpyDeviceToHost);
  printf("Tempg[1000] = %f\n",T[1000]);
  delete[] T;
  ...
  Interaction_Forces(INTERSTEP_Verlet);    //-Interaction.
   ...
   printf("Right Before DtVariable(): \n");
  T = (double*)malloc(sizeof(double)*Np);
  cudaMemcpy(T,Tempg,sizeof(double)*Np,cudaMemcpyDeviceToHost);
  printf("Tempg[1000] = %f\n",T[1000]);
  delete[] T;
...
void JSphGpuSingle::Run(std::string appname,JSphCfgRun *cfg,JLog2 *log){
  ...
  VisuConfig();
  ConfigDomain();
  printf("After ConfigDomain() is called: \n");
  printf("Temp[0] = %f\n",Temp[0]);
  printf("Temp[1000] = %f\n",Temp[1000]);
  double *T;
  T = (double*)malloc(sizeof(double)*Np);
  ...
  while(TimeStep<TimeMax){
    ...
    printf("Right before ComputeStep(): \n");
    double *T;
    T = (double*)malloc(sizeof(double)*Np);
    cudaMemcpy(T,Tempg,sizeof(double)*Np,cudaMemcpyDeviceToHost);
    printf("Tempg[1000] = %f\n",T[1000]);
...
void JSphGpuSingle::SaveData(){
  ...
  JDataArrays arrays;
  AddBasicArrays(arrays,npsave,AuxPos,Idp,AuxVel,AuxRhop,AuxTemp);
