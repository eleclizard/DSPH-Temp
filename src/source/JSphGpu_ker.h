...
typedef struct StrInterParmsg{
  ...
  const double *temp; // For temperature
  const unsigned *idp;
  const typecode *code;
  ...
  float *viscdt;
  float* ar;
  float *atemp; // For temperature derivative
  float3 *ace;
  float *delta;
  tsymatrix3f *gradvel;
  float4 *shiftposfs;
  ...
  StrInterParmsg(
    ...
    ,const float4 *velrhop_, const double *temp_,const unsigned *idp_,const typecode *code_
    ,const float *ftomassp_,const tsymatrix3f *spstau_
    ,float *viscdt_,float* ar_,float *atemp_,float3 *ace_,float *delta_
