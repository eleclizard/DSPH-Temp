...
template<bool floating,bool shift,bool inout> __global__ void KerComputeStepVerlet
  ...
  ,const float4 *velrhop1,const float4 *velrhop2, const double *temp1, const double *temp2
  ,const float *ar,const float *atemp,const float3 *ace,const float4 *shiftposfs,const float3 *indirvel
  ...
  ,double2 *movxy,double *movz,typecode *code,float4 *velrhopnew,double *tempnew)
{
  ...
  if(p<n){
    if(p<npb){ //-Particles: Fixed & Moving.
      ...
      tempnew[p] = temp2[p]; //Temperature: Constant temperature boundary
    }
    else{ //-Particles: Floating & Fluid.
      tempnew[p] = temp2[p] + dt2 * atemp[p]; // Temperature: update temperature (for fluid)
...
void ComputeStepVerlet(bool floating,bool shift,bool inout,unsigned np,unsigned npb
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhopnew,double *tempnew,cudaStream_t stm)
{
  ...
  if(np){
    ...
    if(inout){      const bool tinout=true;
      if(shift){    const bool shift=true;
        if(floating)KerComputeStepVerlet<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,rhopzero,rhopoutmin,rhopoutmax,velrhop1,velrhop2,temp1,temp2,ar,atemp,ace,shiftposfs,indirvel,dt,dt205,dt2,Float3(gravity),movxy,movz,code,velrhopnew,tempnew);
        else        KerComputeStepVerlet<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,rhopzero,rhopoutmin,rhopoutmax,velrhop1,velrhop2,temp1,temp2,ar,atemp,ace,shiftposfs,indirvel,dt,dt205,dt2,Float3(gravity),movxy,movz,code,velrhopnew,tempnew);
...
template<bool floating,bool shift,bool inout> __global__ void KerComputeStepSymplecticPre
  ...
  ,const float4 *velrhoppre,const double *temppre,const float *ar,const float *atemp,const float3 *ace,const float4 *shiftposfs
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,double *temp)
{
  ...
  if(p<n){
    if(p<npb){ //-Particles: Fixed & Moving.
      ...
      velrhop[p]=rvelrhop;
      temp[p] = temppre[p]; // Temperature: Fixed temperature boundary.
    }
    else{ //-Particles: Floating & Fluid.
      const typecode rcode=code[p];
      //-Updates density.
      ...
      temp[p] = temppre[p] + dtm * atemp[p]; // Temperature: Calculate fluid temperature
      if(!floating || CODE_IsFluid(rcode)){ //-Particles: Fluid.
        //-Calculate displacement. | Calcula desplazamiento.
...
void ComputeStepSymplecticPre(bool floating,bool shift,bool inout,unsigned np,unsigned npb
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,double *temp,cudaStream_t stm)
{
  if(np){
    dim3 sgrid=GetSimpleGridSize(np,SPHBSIZE);
    if(inout){      const bool tinout=true;
      if(shift){    const bool shift=true;
        if(floating)KerComputeStepSymplecticPre<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,temppre,ar,atemp,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,temp);
        else        KerComputeStepSymplecticPre<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,temppre,ar,atemp,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,temp);
      }else{        const bool shift=false;
        if(floating)KerComputeStepSymplecticPre<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,temppre,ar,atemp,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,temp);
        else        KerComputeStepSymplecticPre<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,temppre,ar,atemp,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,temp);
      }
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,double *temp)
{
  ...
  if(p<n){
    if(p<npb){ //-Particles: Fixed & Moving.
      ...
      // Temperature: Corrector for temperature calculation
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,double *temp,cudaStream_t stm)
{
  if(np){
    ...
    if(inout){      const bool tinout=true;
      if(shift){    const bool shift=true;
        if(floating)KerComputeStepSymplecticCor<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,temppre,ar,atemp,ace,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,temp);
        else        KerComputeStepSymplecticCor<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,temppre,ar,atemp,ace,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,temp);
      }else{        const bool shift=false;
        if(floating)KerComputeStepSymplecticCor<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,temppre,ar,atemp,ace,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,temp);
        else        KerComputeStepSymplecticCor<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,temppre,ar,atemp,ace,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,temp);
      }
