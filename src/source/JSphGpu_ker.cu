...
template<TpKernel tker,TpFtMode ftmode,bool symm>
  ...
  ,const float4 *poscell,const float4 *velrhop,const double *temp,const typecode *code,const unsigned* idp
  ,float massf,const float4 &pscellp1,const float4 &velrhop1, double &tempp1,float &arp1, float &atempp1,float &visc)
{
  for(int p2=pini;p2<pfin;p2++){
    ...
    if(rr2<=CTE.kernelsize2 && rr2>=ALMOSTZERO){
      ...
      const float frx=fac*drx,fry=fac*dry,frz=fac*drz,fabc=fac; //-Gradients. Add temperature gradient
      ...
      if(compute){
        ...
        //-Temperature derivative (Energy equation)
  ...
  ,float *viscdt,float *ar,float *atemp)
{
  ...
  if(p<n){
    ...
    float visc=0,arp1=0;
    float atempp1 = 0; //Initiate temp derivative.

    //-Loads particle p1 data.
    const float4 pscellp1=poscell[p1];
    const float4 velrhop1=velrhop[p1];
    double tempp1 = temp[p1]; //Load temperature of particle p1
...
template<TpKernel tker,TpFtMode ftmode,bool lamsps,TpDensity tdensity,bool shift,bool symm>
  ...
  ,const float4 &pscellp1,const float4 &velrhop1,float pressp1, double tempp1
  ...
  ,TpShifting shiftmode,float4 &shiftposfsp1)
{
  for(int p2=pini;p2<pfin;p2++){
    ...
    if(rr2<=CTE.kernelsize2 && rr2>=ALMOSTZERO){
      //-Computes kernel.
      const float fac=cufsph::GetKernel_Fac<tker>(rr2);
      const float frx=fac*drx,fry=fac*dry,frz=fac*drz,fabc=fac; //-Gradients. Add in gradient for temperature.
      ...
      if (compute) {
        ...
        const float tempConst = (4 * massp2*CTE.HeatKFluid*heatKp2) / (CTE.HeatCpFluid *velrhop1.w*rhopp2*(CTE.HeatKFluid + heatKp2));
        atempp1 += float(tempConst*dtemp*fabc);
      }
...
{
  printf("Inside KerInteractionForcesFluid(): \n");
  //printf("ace.x[1000] = %f\n",*ace.x[1000]);
  printf("temp[1000] = %f\n",temp[1000]);
  ...
  if(p<n){
    ...
    float atempp1 = 0.; //Initiate temp rate of change for particle p1
    ...
    const float4 pscellp1=poscell[p1];
    const float4 velrhop1=velrhop[p1];
    //printf("Step before temp access Done\n");
    //double tempp1=temp[p1]; //Temperature data for particle p1.
    double tempp1=temp[p1]; //Temperature data for particle p
    //printf("Step after temp access Done\n");
    ...
    for(int c3=ini3;c3<fin3;c3+=nc.w)for(int c2=ini2;c2<fin2;c2+=nc.x){
      ...
      if(pfin){
        ...
        if(symm && rsymp1)KerInteractionForcesFluidBox<tker,ftmode,lamsps,tdensity,shift,true > (false,p1,pini,pfin,viscof,ftomassp,tauff,poscell,velrhop,temp,code,idp,CTE.massf,ftp1,pscellp1,velrhop1,pressp1,tempp1,taup1_xx_xy,taup1_xz_yy,taup1_yz_zz,grap1_xx_xy,grap1_xz_yy,grap1_yz_zz,acep1,arp1,atempp1,visc,deltap1,shiftmode,shiftposfsp1); //<vs_syymmetry>
      }
    }
    ...
    for(int c3=ini3;c3<fin3;c3+=nc.w)for(int c2=ini2;c2<fin2;c2+=nc.x){
      unsigned pini,pfin=0;  cunsearch::ParticleRange(c2,c3,ini1,fin1,begincell,pini,pfin);
      if(pfin){
                        KerInteractionForcesFluidBox<tker,ftmode,lamsps,tdensity,shift,false> (true ,p1,pini,pfin,viscof,ftomassp,tauff,poscell,velrhop,temp,code,idp,CTE.massf,ftp1,pscellp1,velrhop1,pressp1,tempp1,taup1_xx_xy,taup1_xz_yy,taup1_yz_zz,grap1_xx_xy,grap1_xz_yy,grap1_yz_zz,acep1,arp1,atempp1,visc,deltap1,shiftmode,shiftposfsp1);
      if(symm && rsymp1)KerInteractionForcesFluidBox<tker,ftmode,lamsps,tdensity,shift,true > (true ,p1,pini,pfin,viscof,ftomassp,tauff,poscell,velrhop,temp,code,idp,CTE.massf,ftp1,pscellp1,velrhop1,pressp1,tempp1,taup1_xx_xy,taup1_xz_yy,taup1_yz_zz,grap1_xx_xy,grap1_xz_yy,grap1_yz_zz,acep1,arp1,atempp1,visc,deltap1,shiftmode,shiftposfsp1);
      }
    }
    ...
    if(shift||arp1||acep1.x||acep1.y||acep1.z||visc){
      ...
      ar[p1]+=arp1;
      atemp[p1]+=atempp1; //Store temp rate of change array
  ...
  void Interaction_ForcesT_KerInfo(StKerInfo *kerinfo)
{
  ...
  {
    typedef void (*fun_ptr)(unsigned,unsigned,float,float,int,int4,int3,const int2*,unsigned,const unsigned*,const float*,const float2*,float2*,const float4*,const float4*,const double*,const typecode*,const unsigned*,float*,float*,float*,float3*,float*,TpShifting,float4*);
  ...
  {
    typedef void (*fun_ptr)(unsigned,unsigned,int,int4,int3,const int2*,const unsigned*,const float*,const float4*,const float4*,const double*,const typecode*,const unsigned*,float*,float*,float*);
...
{
  //printf("Inside Interaction_ForcesGpuT: \n");
  /*printf("parms.posz[1000] = %f\n",t.posz[1000]);
  printf("parms.temp[1000] = %f\n",t.temp[1000]);*/
  //printf("parms.ace.x[1000] = %f\n",t.ace.x[1000]);
  //-Collects kernel information.
#ifndef DISABLE_BSMODES
  if(t.kerinfo){
  ...
  if(t.fluidnum){
    ...
    if(t.symmetry) //<vs_syymmetry_ini>
      ...
      ,t.ftomassp,(const float2*)t.tau,(float2*)t.gradvel,t.poscell,t.velrhop,t.temp,t.code,t.idp
      ,t.viscdt,t.ar,t.atemp,t.ace,t.delta,t.shiftmode,t.shiftposfs);
  ...
  if(t.boundnum){
    ...
    if(t.symmetry) //<vs_syymmetry_ini>
      ...
      (t.boundnum,t.boundini,dvd.scelldiv,dvd.nc,dvd.cellzero,beginendcell+dvd.cellfluid,t.dcell
        ,t.ftomassp,t.poscell,t.velrhop,t.temp,t.code,t.idp,t.viscdt,t.ar,t.atemp);
    else //<vs_syymmetry_end>
...
void Interaction_Forces(const StInterParmsg &t){
 ...
 #endif
#endif
/*printf("Inside cusph::Interaction_Forces():\n");
printf("parms.poszg[1000] = %f\n",t.posz[1000]); //posz is a pointer in the original code
printf("parms.temp[1000] = %f\n",t.temp[1000]);*/
}
