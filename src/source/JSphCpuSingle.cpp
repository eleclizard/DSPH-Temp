...
void JSphCpuSingle::ConfigDomain(){
  ...
  for (unsigned p = 0; p<Np; p++)
    Tempc[p] = HeatTempFluid; //Assigning every point fluid temperature initially
...
void JSphCpuSingle::RunCellDivide(bool updateperiodic){
  ...
  if(TStep==STEP_Verlet){
    ...
    CellDivSingle->SortArray(TempM1c); // For temperature
  }
    ...
    CellDivSingle->SortArray(TempPrec); // For temperature
  }
...
void JSphCpuSingle::Run(std::string appname,JSphCfgRun *cfg,JLog2 *log){
  ...
  while(TimeStep<TimeMax){
    ...
    /*printf("The following information for 8 bytes (double): \n");
    printf("Count = %d\n",ArraysCpu->Arrays8b->Count);
    printf("CountUsed = %d\n",ArraysCpu->Arrays8b->CountUsed);*/
  }
...
void JSphCpuSingle::SaveData(){
  ...
  tfloat3 *vel=NULL;
  float *rhop=NULL;
  double *temp=NULL; // Temperature: Declare temporal array
  if(save){
    ...
    unsigned npnormal=GetParticlesData(Np,0,PeriActive!=0,idp,pos,vel,rhop,temp,NULL); // Temperature: Add in new temp parameter.
